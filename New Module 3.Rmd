...
hist(uniform_p, main = "Simulated Values from a Uniform Prior", xlab = 'Values from "Negative Infinite" (-100,000) to "Positive Infinite" (100,000)') # Visualize the simulated values
```
...
Like the frequentist confidence interval, __the Bayesian credible interval expresses uncertainty in regards to the value of a parameter estimate__ (Kruschke & Liddell, 2018). The larger the spread of the posterior distribution of a parameter, the wider the credible interval, the less certain we are (and vice versa). 
...
## Recommended Introductory-Level Bayesian Journal Articles
